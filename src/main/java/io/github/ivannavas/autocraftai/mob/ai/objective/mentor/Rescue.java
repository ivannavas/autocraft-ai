 ...
 *
 * <p>And one answer that is not a lesson at all. Asked about an objective that has got nowhere for
 * minutes, the mentor may decide the objective is the problem — the tool is missing, the place is wrong —
 * and say so in {@link #replan()}. The run then gives the objective up and asks the planner again, with
 * the mentor's sentence in the question, which is the one thing a table cannot be taught.
 *
...
public record Rescue(MentorAsk.Reason reason, String pursuit, String state, List<Lesson> lessons,
                     ...
                     String craftKey, List<Lesson> craftLessons, String replan) {
    public Rescue {
    ...
    public boolean isEmpty() {
        return lessons.isEmpty() && passageLessons.isEmpty() && craftLessons.isEmpty() && replan.isEmpty();
    }
